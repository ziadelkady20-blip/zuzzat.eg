...
const typeVariant: Record<Order["type"], "blue"|"green"|"orange"> = {
  "dine-in": "blue", pickup: "green", delivery: "orange"
};
            ...
            {orders.map(order => (
              <tr key={order.id} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  {order.type === "dine-in" ? `Table ${order.tableId}` : order.customerName}
                </td>
                <td className="px-4 py-3">
                  <Badge variant={typeVariant[order.type]}>
